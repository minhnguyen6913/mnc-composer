...
class XLSXWriter
{
    ...
    public function setRow($sheet_name, $row){
        $this->initializeSheet($sheet_name);
        $this->sheets[$sheet_name]->row_count = $row;
    }
    protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false, $startRow = 0 )
    {
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $col_options = null, $startRow = 0)
    {
        ...
        self::initializeSheet($sheet_name, $col_widths, $auto_filter, $freeze_rows, $freeze_columns, $startRow);
        $sheet = &$this->sheets[$sheet_name];
    ...
    public function writeSheetPartial(int $startRow, array $data, $sheet_name='', array $header_types=array())
    {
        $sheet_name = empty($sheet_name) ? 'Sheet1' : $sheet_name;
        $data = empty($data) ? array(array('')) : $data;
        $this->setRow($sheet_name, $startRow);
        if (!empty($header_types))
        {
            $this->sheets[$sheet_name]->columns =  $this->initializeColumnTypes($header_types);
            if (!isset($header_types[0])) {
                $header_row = array_keys($header_types);
                $sheet = &$this->sheets[$sheet_name];
                $sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="0" r="' . ($sheet->row_count+1) . '">');
                foreach ($header_row as $c => $v) {
                    $this->writeCell($sheet->file_writer, $sheet->row_count, $c, $v, $number_format_type='n_string', 0);
                }
                $sheet->file_writer->write('</row>');
                $sheet->row_count++;
            }
        }else {
            $this->sheets[$sheet_name]->columns = [];
        }
